...
def draw_distant_stops(k: int):
    points = ad.distant_stops(k)
    m = folium.Map(location=[52, 21])
    for point in points:
        plot_dot(point['a_nazwa_zespolu'], point['a_slupek'],
                 str(point['intersect']),
                 m,
                 point['a_szer_geo'],
                 point['a_dlug_geo'],
                 3,
                 4,
                 f"Matched with {point['b_nazwa_zespolu']} \
                   with {point['intersect']} lines"
                 )
        plot_dot(point['b_nazwa_zespolu'], point['b_slupek'],
                 str(point['intersect']),
                 m,
                 point['b_szer_geo'],
                 point['b_dlug_geo'],
                 3,
                 4,
                 f"Matched with {point['a_nazwa_zespolu']} \
                   with {point['intersect']} lines"
                 )

    m.save(f"{os.getcwd()}/DATA/MAPS/distant_stop.html")
